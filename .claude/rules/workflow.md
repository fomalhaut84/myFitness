...
**봇이 돌 수 없는 경우**(쿼터 소진 등)는 일반 PR 에 한해 `봇: 미실행 (사유, YYYY-MM-DD)` 를 적고
회복 후 `@codex review` — **릴리즈 PR 은 봇 회복까지 대기한다** (릴리즈는 곧장 실서비스로 가고
핫픽스와 달리 긴급성이 없다). **매 리뷰 결과는 사용자에게 요약 보고.**
