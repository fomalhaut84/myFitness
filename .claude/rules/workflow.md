...
> **정정 (2026-09-11 · pleiades#58 · 1a-2).** 위 문장의 *"vitest 가 아니라 verify 스크립트 2개"* 는 1a-2 로 소진됐다 —
> `npm run test` 는 이제 **`vitest run`(`src/**/__tests__/**/*.test.ts` · 회귀 baseline) + verify 스크립트 2개**다. 4종 검증 세트는 그대로. 되돌리기: 즉시.
...
| PR | 봇 불가 시 |
|---|---|
| **일반 PR · self-review 경로** (8-0: 문서·config 소규모·시안) | **self-review + 7단계 검증으로 완료.** 봇을 기다리지 않는다. `봇: 미실행 (사유, YYYY-MM-DD)` 를 적고 회복 후 `@codex review` |
| **일반 PR · 에이전트 필수 경로** (8-0: 애플리케이션 코드·DB·API/MCP) | **8-1 사전 에이전트 리뷰(critical/major = 0)가 완료 판정.** 사전 리뷰도 생략한 PR 은 머지하지 않는다. `봇: 미실행 (사유, YYYY-MM-DD)` 표기, 회복 후 `@codex review` |
...
- 회귀 테스트는 vitest 로 `src/**/__tests__/**/*.test.ts` 에 쓴다 (1a-2 · pleiades#58). 프레임워크로 잡기 어려운 것만 재현 스크립트 (`scripts/*.ts`) + `docs/specs/` 스펙 갱신으로 대체.

> **정정 (2026-09-11 · pleiades#58 · 1a-2).** 이전 원칙은 *"테스트 프레임워크 부재 시 … 스크립트로 대체"* 였다. vitest 도입 뒤에도 남겨두면
> 회귀 테스트를 계속 스크립트로 대체하게 만드는 **살아 있는 지시**라 교체했다(pleiades 1a-2 재감사 정정 4). 되돌리기: 즉시.
