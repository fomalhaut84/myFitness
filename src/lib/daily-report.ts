...
- **개인 목표 진행 상황** (시스템 프롬프트의 "개인 목표" 섹션에 값이 있을 때만): 각 목표별 현재 vs 목표 gap/진행률 한 줄 언급
...
- 경고 규칙에 해당하면 반드시 경고 포함
- **개인 목표 진행 상황** (시스템 프롬프트의 "개인 목표" 섹션에 값이 있을 때만): 오늘 훈련이 목표 (평균 페이스/주간 거리/체중 등) 에 어떻게 기여했는지 한 줄 코멘트
