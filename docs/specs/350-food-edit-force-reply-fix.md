...
- [ ] F7: 완료/취소 메시지에 `remove_keyboard` 부착 — 기존에 박힌 force_reply 상태 정리 시도 — **실기기 결과 무효 (2026-09-04)**, §7 참조
...
- 이미 클라이언트에 박힌 force_reply 의 확정적 제거 — Bot API 에 회수 수단 없음. F7 의 `remove_keyboard` 는 best-effort 이며, 실패 시 사용자가 해당 프롬프트에 답장하거나 메시지를 직접 삭제해야 함
    - **2026-09-04 실기기 확인: F7 은 동작하지 않았다.** v2.27.2 배포 후에도 채팅방 재진입 시 입력폼이 계속 재생성됐고, 사용자가 **옛 프롬프트 메시지를 직접 삭제**해서야 해소됐다. `remove_keyboard` 는 best-effort 가 아니라 **이 목적에는 사실상 무효**로 보는 것이 맞다 (ReplyKeyboardRemove 와 ForceReply 는 클라이언트에서 별개 상태).
    - 이때 **코드 회귀로 오진하지 말 것** — 배포(v2.27.2~4 전부 success)·번들(`build:bot` 포함)·발송지점(`src/` 내 force_reply 0건) 이 모두 정상인데도 증상은 재현된다. 증상이 남으면 인용된 대상 메시지의 **발송 시각**부터 확인한다. 신규 프롬프트는 force_reply 를 쓰지 않으므로 새로 박힐 일은 없고, 재발 조건도 없다.
