...
- Codex bot 4회차 (자동 재리뷰): P0/P1 0 · P2 2 → **미반영, 후속 #408**. 둘 다 기록 시작일이 걸린 첫 버킷 1개에만 해당 — (1) 커버리지 분모가 달력 전체 일수 (프로덕션 2020-06 은 15/30 = 0.5 로 경계 통과. `/history` 의 `coverableDays` 공용화로 해결) (2) 주 버킷이 하한 이전 날짜를 조회 (하한 = 실제 최초 기록일이라 그 앞에 행이 없다). `/history` · summary API 와 같은 뿌리라 #405 와 함께 처리
- 릴리즈 PR #409 Codex 1회차: P0/P1 0 · P2 1 → **#396 에 편입**. 최고/최저 판독값의 `/history` 링크 (`bucketHref`) 가 선택 지표를 싣지 않아 기본 지표로 열림 — #396 의 "포인트 → 일 뷰 링크" 와 같은 경로
- **최종**: 사전 critical/major 0/0 · 봇 P0/P1 0/0 · 봇 P2 7건 (반영 4 · 후속 #408 2 · #396 1)
- **배포 후 확인 (v2.32.0, 2026-09-21)**: 사용자 실데이터 확인 완료 (PR #409 검증 계획). 최저~최고 띠는 제거 요청이 없어 그대로 둔다
