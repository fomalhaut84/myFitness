...
- [ ] **F6**: **Archived plan 이력** (하단 섹션)
  - 최신순 리스트: startDate ~ endDate + targetDistance? + status
  - 클릭 시 상세 (같은 페이지 확장 or 별도 sub-page)
...
- [ ] **A3**: `GET /api/training-plan/[planId]` — 특정 plan 상세 (archived 열람용).
- [ ] **A4**: `GET /api/recommend-today` — `recommendTodayWorkout` 파싱해 반환.
