...
- **스코프**:
  ...
  2. **Write 경로 6곳 propagate**: `POST /api/food` (JSON + photo), `bot/food.ts`, `bot/food-photo.ts`, `backfill.ts` — 각 estimator 결과에 source 태그 (mfds/ai/vision) 붙여 저장. `repeat-lookup` hit 는 원본 items[i].source 를 그대로 전파. `log.viaRepeat=true` 세팅은 **실제로 hit 의 kcal / macros / items 중 하나 이상이 write 에 채택된 경우만** — hit 발생 자체가 아니라 채택 여부 기준. backfill 에서 partial hit 이 rejected 되고 MFDS/AI 로 채워진 경우 viaRepeat=false 유지.
     - **Replacement paths (Codex P2 재⁴지적)**: `PATCH /api/food/[id]/route.ts` + `bot/food-edit-callback.ts:364-376` (description/mealType 변경 시 kcal/macros/items 리셋) 도 write path. 이 경로에서 재추정 대기 상태 로그가 stale `viaRepeat=true` 유지되면 재추정 지연/실패 시 empty log 가 무한 "재사용" 라벨. **description/mealType 변경 시 viaRepeat=false 로 atomically reset** 필수. Non-repeat backfill 이 later 채우면 그 write 에서도 viaRepeat=false 명시 write (fallback).
