...
| 후보 | 내용 | 비고 |
|---|---|---|
| **#418 HRR** (feature · P2) | 러닝 종료 후 심박 회복 — `HeartRateRecord.rawData.heartRateValues` (2분 간격 684샘플/일) 로 종료 후 +1/+2/+5/+10분 곡선 + HRR 을 활동 상세에, 이력이 쌓이면 `/insights` 5번째 패널 | 사용자 직접 요청 (2026-09-22). API 호출 0. 이슈에 프로덕션 확인 쿼리 2개 (`recoveryTime` 존재 · heartRateValues 시작일). UI → 디자인 단계 |
...
| D8 잔여 | 케이던스 · 보폭 추세 (`/trends` 지표 등록 1건씩) · 수면 점수 → 다음날 페이스 · 요일/시간대 · 스트레스/바디배터리 · 수면 스택/SpO2 (HRV 2026-04~) | `docs/specs/m15-overview.md` D8 표 · #397 체크리스트 |
