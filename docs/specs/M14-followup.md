...
## 현재 상태 (2026-09-23 밤, #446 · #447 머지 · v2.37.0 릴리즈 준비)

**최근 릴리즈:** v2.36.1. dev 에 **#440 (PR #446) · #442 (PR #447)** 머지 완료 (2026-09-23 23:34 KST) — 릴리즈 PR **v2.37.0** (feature 2건 · migration 없음 · 패키지 변경 없음) 대기. M17-1 · M17-3 완료, M17-2 (#441) 는 사용자 실험 대기.

### 인계 (다음 세션에서 이어갈 것)

| 항목 | 상태 | 비고 |
|---|---|---|
| **v2.37.0 릴리즈** | 릴리즈 PR 생성 → 봇 게이트 → 사용자 머지 → 태그 · Release | 배포 후 확인: 활동 상세 AI 평가 1회 (스플릿 API · 세션 채널 `activity-eval` · pm2 로그 `[activity-eval]`) · `/trends?metric=hrr2` 캡션 "2026-04 부터" · 개인 기록 행 |
| **#445 월별 그리드 월요일 시작** (P2) | 다음 스펙 (사용자 요청) | `month-cells.ts` · `MonthGrid` · `YearMonthCard` · `MonthlyHeatmap` · `WeeklyChart`. 주간 버킷은 이미 월요일 |
| **#441 HRR 1분 해상도** (P2) | F1 기준값 완료 (전 월 60초 0건 · 2분 격자) · **F2 워치 "매초" 실험 대기** | 결과 나오면 스펙 §5 기록 → 소스 있으면 §4 구현, 없으면 종료 |
| **#444 이브닝 리포트 · MCP `get_activity_context`** (P2) | 신규 | #440 의 `buildEvalContext` 를 도구로 |
