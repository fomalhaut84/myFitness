...
**최근 릴리즈:** **v2.38.0** (릴리즈 PR #457 · merge commit · 2026-09-24). #444 Phase 1 (PR #456) + 릴리즈 PR Codex P2 1건 (fix PR #458 · `hrrDrop10` 안내 단위). M17-5 완료. dev = main (이 문서 PR 만 앞섬).

### 인계 (다음 세션에서 이어갈 것)

| 항목 | 상태 | 비고 |
|---|---|---|
| **v2.38.0 배포 후 확인** | Deploy on Release 실행 (run 35943389287) | 첫 이브닝 리포트가 `get_activity_context` 호출 · 모닝 혈압 항목 · 주간 endDate (릴리즈 노트 검증 계획). 결과에 따라 프롬프트 미세 조정 또는 #455 착수 |
