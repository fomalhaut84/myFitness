...
## 현재 상태 (2026-09-23 밤, v2.36.1 배포 후 확인 · 보존 창 감사 완료 · 세션 종료)
...
| 후보 | 내용 | 비고 |
|---|---|---|
| **#440 러닝 상세 AI 평가 확장** (feature · **P1**) | 현재 `requestAiEval` 은 6개 지표 · "3줄 이내". 상세 페이지 전 섹션 (기본 · 강도 · 종료 후 회복 · 다이나믹스 · 추가 지표 · 환경 · 같은 코스/비슷한 기록 비교) 을 근거로 — 컨텍스트 조립을 서버 순수 함수로, MCP 도구 경계 결정 | UI 소규모. M17 후보 1순위 |
