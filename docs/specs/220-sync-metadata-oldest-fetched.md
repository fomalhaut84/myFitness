...
**커버 범위 = `[oldestFetchedDate, coveredThroughDate]` (contiguous)**.

Codex bot 3 라운드 P2 (PR #218, #239, #240) 를 거쳐 도달한 최종 semantic:
- 실제 sync API 호출 range 만 반영 (record 존재 ≠ API 커버)
- 두 필드는 **항상 함께 관리** — contiguous 하나의 range 만 표현
- 새 fetch [s, e] 의 UPDATE 규칙:
  1. 기존 null → 초기화 `[s, e]`
  2. adjacent 또는 overlap (`s <= through+1d AND e >= oldest-1d`) → merge `[min(s,oldest), max(e,through)]`
  3. disjoint 이지만 recent (`e >= today-1d`) → reset `[s, e]` (recent contiguous coverage 를 canonical, 옛 range 는 stale)
  4. disjoint + old → 무시 (기존 marker 유지)

shortfall 판정 (backfill 유도):
- 마이그레이션 (`oldest` 또는 `through` null): `firstRecordDate !== null` 시 shortfall (record 없는 계정은 skip)
- lower bound: `oldest > requiredStart` → 과거 커버 부족
- upper bound: `through < today-1d` → 최근 gap 존재 (예: /api/sync 로 옛 disjoint range 만 fetch 된 상태)

### 수렴 시나리오

| 시나리오 | 상태 | 동작 |
|---|---|---|
| pre-#220 record + 어떤 데이터든 존재 | `oldest=null, through=null, first=D-N` | shortfall: first != null → true → backfill D-90 → `[D-90, today]` 초기화 (규칙 1) |
| 신규 계정 (record 없음, 성공 sync) | `oldest=null, first=null` | shortfall=false (무한 API 호출 방지). 다음 incremental sync 로 `[lastSyncDate+1, today]` 초기화 |
| 마이그레이션 이후 정상 사이클 | `[D-90, today]` | requiredStart=D-90 → false. through=today → false. no shortfall |
| Codex PR #240 P2-1 재현 (오래된 record + 짧은 sync) | `[null, null], first=D-365` | shortfall → backfill → `[D-90, today]` 정착. D-365 로 오인 X |
| Codex PR #240 P2-2 재현 (`/api/sync` 옛 range 후 cron 최근) | `[D-500, D-450]` → cron [D-1, today] | disjoint + recent → **reset** → `[D-1, today]`. 다음 preSync: oldest=D-1 > D-90 → shortfall → backfill → `[D-90, today]` |
| `/api/sync` 옛 range 후 cron adjacent | `[D-500, D-450]` → cron [D-449, today] | adjacent → merge → `[D-500, today]` (완전한 coverage) |
| `/api/sync` 매우 옛 range 만 (`[D-500, D-450]`), cron 미실행 | `[D-500, D-450]` | through=D-450 < today-1d → shortfall → backfill 유도 |
