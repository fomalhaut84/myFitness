...
| 시점 | 상태 | 동작 |
|---|---|---|
| pre-#220 record + 90일 데이터 충분 | `oldest=null`, `first=D-100` | fallback → shortfall=false. sync 후 seed 로 `oldest=min(D-100, startDate)` 정착 |
| pre-#220 record + 90일 부족 | `oldest=null`, `first=D-30` | fallback → shortfall=true → backfill (startDate=D-90). seed 로 `oldest=min(D-30, D-90)=D-90` 정착 |
| 신규 계정 (record 없음, 성공 sync) | `oldest=null`, `first=null` | fallback → shortfall=false (Codex 정책). 다음 syncAll 로 `oldest=startDate` (신규 데이터 도착 시) |
| 마이그레이션 이후 정상 사이클 | `oldest=D-90` | requiredStart=D-90 → shortfall=false. 시간 흐름에 따라 새 window 요구 시 자연 backfill |

### 회귀 안전성

- `updateSyncMetadata` 는 sync.ts 내부 non-exported 함수. 유일 호출부 line 314 만 새 signature 적용.
