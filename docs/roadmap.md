...
## M15-1: 집계 기반 — KST 버킷 · 지표 레지스트리 · history summary API — 우선순위 ★★★ (코드 ✅ · F12 성능 기준 미달)
...
- [ ] F12 6년 `year` 요청 1s 이내 — **미달 (웜 1.1s)**. 캐시(#394) 적용 후 재측정해야 M15-1 완료
- 이슈: #393 (코드 완료 · PR #399 · v2.30.0)

## M15-2: `/history` 연 · 월 · 일간 종합 — 우선순위 ★★★

- [ ] 레벨 공용 네비 (이전/다음 + 점프) · 브레드크럼
- [ ] 연 뷰 (12개월 카드 + 지표 선택 + 연 KPI) · 월 뷰 (값 셀 그리드 + 월 KPI + 일별 스트립)
- [ ] 일간 종합 페이지 8 섹션 (활동 · 수면 · 심박 · 체성분 · 혈압 · 걸음/칼로리 · 식단 · AI 리포트)
- [ ] 초기 지표: 러닝 km · 걸음 · 수면 점수 · 안정시 심박 · 체중
- 이슈: #394

## M15-3: `/trends` 추이 분석 — 우선순위 ★★

- [ ] 시계열 (합계 막대 / 평균 선+밴드) · 전년 동기 겹침 · 계절성 · 기간 비교
- 이슈: #395

## M15-4: 하이라이트 — 우선순위 ★★

- [ ] 개인 기록 패널 · 이벤트 마커 (MetricChange · 플랜 · 레이스) · 포인트 → 일 뷰 링크 · 커버리지 띠
- [ ] `Activity.eventType` 컬럼 승격 + rawData 백필
- 이슈: #396

## M15-5: 심화 시각화 — 우선순위 ★

- [ ] 효율 산점도 · HR 존 분포 · 기상 vs 페이스 · 수면 규칙성 · 교차 상관 중 선별
- 이슈: #397
